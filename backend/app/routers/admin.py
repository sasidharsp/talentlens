...
def get_proctoring_config(db: Session = Depends(get_db), current_user=Depends(require_any_staff)):
    cfg = _get_or_create_config(db)
    return _cfg_dict(cfg)

@router.put("/proctoring-config")
def update_proctoring_config(payload: dict, db: Session = Depends(get_db), current_user=Depends(require_admin)):
    cfg = _get_or_create_config(db)
    fields = ['enabled','max_weight','grace_frames','cooldown_ms','audio_rms',
              'audio_hold_ms','phone_confidence','phone_frames','phone_term_count',
              'gaze_h','gaze_v_up','gaze_v_down','head_thresh','violation_weights']
    for f in fields:
        if f in payload: setattr(cfg, f, payload[f])
    cfg.updated_by = current_user.id
...
def reset_proctoring_config(db: Session = Depends(get_db), current_user=Depends(require_admin)):
    cfg = _get_or_create_config(db)
    db.delete(cfg); db.commit()
    return _cfg_dict(_get_or_create_config(db))
